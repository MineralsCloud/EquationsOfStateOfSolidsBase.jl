...
[EquationsOfStateOfSolidsBase](https://github.com/MineralsCloud/EquationsOfStateOfSolidsBase.jl).
...
## Install EquationsOfStateOfSolidsBase
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("EquationsOfStateOfSolidsBase")
   ```

3. Run

   ```julia-repl
   julia> using EquationsOfStateOfSolidsBase
   ```
...
version of EquationsOfStateOfSolidsBase, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/EquationsOfStateOfSolidsBase.jl"
```

in the second step above.

## Update EquationsOfStateOfSolidsBase
...
our [GitHub repository](https://github.com/MineralsCloud/EquationsOfStateOfSolidsBase.jl)
for new releases.
Once we release a new version, you can update EquationsOfStateOfSolidsBase by typing

```@repl
using Pkg
Pkg.update("EquationsOfStateOfSolidsBase")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall EquationsOfStateOfSolidsBase
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("EquationsOfStateOfSolidsBase")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall EquationsOfStateOfSolidsBase.
