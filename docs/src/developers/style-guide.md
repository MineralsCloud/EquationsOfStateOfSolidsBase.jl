...
!!! info
    The style guide is always a work in progress, and not all EquationsOfStateOfSolidsBase code
    follows the rules. When modifying EquationsOfStateOfSolidsBase, please fix the style violations
...
EquationsOfStateOfSolidsBase uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/EquationsOfStateOfSolidsBase.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the EquationsOfStateOfSolidsBase directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to EquationsOfStateOfSolidsBase have
